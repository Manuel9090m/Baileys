...
const makeSocket = ({ waWebSocketUrl, connectTimeoutMs, logger, agent, keepAliveIntervalMs, version, browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, syncFullHistory, transactionOpts, qrTimeout, options, }) => {
    ...
    const sendRawMessage = async (data) => {
        try {
            ...
            // Si el error es por conexión cerrada, no hacer nada
            if (error.message.includes('Connection Closed')) {
                return; // Evita que aparezca en la consola
            }
            console.error(error); // Muestra otros errores
        }
    };    
    /** send a binary node */
    ...
    const awaitNextMessage = async (sendMsg) => {
        try {
            ...
            // Opcional: Puedes loguearlo en un archivo en lugar de la consola
            return; // Evita que siga ejecutando si la conexión está cerrada
        }
        ...
        const result = (0, Utils_1.promiseTimeout)(connectTimeoutMs, (resolve, reject) => {
            ...
            onClose = () => reject(new Error('Mensaje no recibido'));
            ws.on('frame', onOpen);
            ws.on('close', onClose);
            ws.on('error', onClose);
        }).finally(() => {
